...
Bullet::Bullet(string _name, int _x, int _y, int _seta, Tank* pTank, int _kind)
...
Modify Bullet::collision(Modify _modify, Object* target) {
	...
	if (target->name == "Tank") {
		...
		if (target == (Object*)p) {
			if (kind !=1) {
				if (lifeTime < FPS / 2 && abs(seta - target->seta) < 90)
					return _modify;
			}
		}
		if (kind == 1 && lifeTime < FPS * 3)return _modify;
		target->explode();
		this->explode();
	}
	if (target->name == "Wall") {
